...
def write_html(sorted_tables: Dict[Tuple[str, str], str], output_html_file: Path):
    ...
    html_template = (
        ...
        "<html>"
        "<title>Songs in IIDX {version_number} {version_name} Not in Infinitas</title>"
        "<head><style>\n"
        "{css}"
        "\n"
        "</style></head>\n"
    ...
    log.info(f"Writing html to {output_html_file}")
    with open(output_html_file, "wt") as html_writer:
